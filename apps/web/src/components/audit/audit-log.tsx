...
export function AuditLog({ log, isLast = false }: AuditLogProps) {
  ...
  const renderDiff = () => {
    ...
    if (log.action === "CREATE") {
      return (
        <div className="mt-2 text-sm">
          <div className="text-muted-foreground">Valores iniciais:</div>
    ...
    if (log.action === "DELETE") {
      return (
        <div className="mt-2 text-sm">
          <div className="text-muted-foreground">Dados deletados:</div>
          <pre className="mt-1 text-xs bg-muted p-2 rounded overflow-x-auto">
      ...
      <div className="flex-1 pt-0.5">
        <div className="flex items-start justify-between gap-2">
          <div>
            ...
            <p className="mt-1 text-sm font-medium">{log.description}</p>
          </div>
        </div>
